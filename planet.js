...
const renderPlanet = planet => {
  ...
  testfilm.textContent = planet?.films
  testresidents.textContent = planet?.residents

 //something is null before creating list
  //const charsLis      = film?.characters?.map(character => `<li><a href="/character.html?id=${character.id}">${character.name}</li>`)
  const residentslist = planet?.characters?.map(character => `<li><a href="/character.html?id=${character.id}">${character.name}</li>`)
